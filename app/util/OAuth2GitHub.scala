...
class OAuth2GitHub(application: Application) {
...
object OAuth2GitHub extends Controller {
  lazy val oauth2 = new OAuth2GitHub(Play.current)
    ...
    (for {
      ...
      if (state == oauthState) {
        oauth2.getToken(code).map { accessToken =>
          Redirect(util.routes.OAuth2GitHub.success()).withSession("oauth-token" -> accessToken)
        }.recover {
